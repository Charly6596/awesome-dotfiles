...
local bar_theme_name = theme_manager.get_active_bartheme()
--------------------------------------------------------------------------------
...
keyboard_config = {}
keyboard_config.menu = {}
keyboard_config.cmd = 'setxkbmap'
keyboard_config.layout = { { "us", "" , "US" }, { "es", "" , "ES" } }
keyboard_config.current = 1;

for k,layout in pairs(keyboard_config.layout) do
    local layout_menu_table = {
        layout[1],
        function()
            keyboard_config.current = layout[1]
            if(os.execute( keyboard_config.cmd .. " " .. layout[1] .. ",us " .. layout[2] )) then
                notification = naughty.notify({
                    title = "Keyboard layout changed:",
                    text = "Selected " .. keyboard_config.current,
                    icon = beautiful.keyboard_icon,
                    timeout = 5,
                    replaces_id = last_keyboard_layout_notification_id
                })
                last_keyboard_layout_notification_id = notification.id
            end
        end
    }
    table.insert(keyboard_config.menu, layout_menu_table)
end


themes_menu = {}

for k, theme_name in pairs(theme_manager.get_themes()) do
    local theme_table = {
        theme_name,
        function()
    ...
    table.insert(themes_menu, theme_table)
end

bar_themes_menu = {}

for k, bar_theme_name in pairs(theme_manager.get_barthemes()) do
    local bar_theme_table = {
        bar_theme_name,
        function()
            theme_manager.set_active_bartheme(bar_theme_name)
            awesome.restart()
        end
    }
    table.insert(bar_themes_menu, bar_theme_table)
end

local function random_theme_option()
...
myawesomemenu = {
    ...
    { "Main theme", themes_menu, beautiful.home_icon },
    { "Bar theme", bar_themes_menu, beautiful.home_icon },
    { "Random theme", random_theme_option, beautiful.appearance_icon},
...
mymainmenu = awful.menu({ items = {
    ...
    { "Layout", keyboard_config.menu, beautiful.keyboard_icon },
